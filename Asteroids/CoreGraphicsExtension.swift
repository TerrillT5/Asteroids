...
extension CGFloat {
    static func random(in range: Range<CGFloat>) -> CGFloat {       // generates a random number in a certain range
...
extension CGRect {
    ...
    init(sqareCenteredAt center: CGPoint, size: CGFloat) {
        let origin = CGPoint(x: center.x - size / 2 , y: center.y - size / 2)
        self.init(origin: origin, size: CGSize.square(size))
    }
    
    var randomPoint: CGPoint {      // generates a random point inside the rectangle to put asteroids
